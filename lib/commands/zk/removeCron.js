...
Command.prototype.handle = function(agent, comd, argv, rl, client, msg) {
	...
	var cmd = {command: module.exports.commandId, cron: args};
	var serverId;
	var serverType;
	var fails = [];
				...
				client.getChildren(function(err, rs) {
					if(!rs.length) {
						...
						var latch = countDownLatch.createCountDownLatch(rs.length, {timeout: 10 * 1000}, function() {
							if(!fails.length) {
								util.log('removeCron servers successfully for serverType: ' + serverType);
							} else {
								util.log('removeCron failed with servers: %j', fails);
							}
							rl.prompt();
						});
