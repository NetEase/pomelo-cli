...
Command.prototype.handle = function(agent, comd, argv, rl, client, msg) {
	...
	var cmd = {command: module.exports.commandId, blacklist: args};
	var fails = [];
				...
				if(err) {
		      	...
		      	if(!res.length) {
		      		...
		      		var latch = countDownLatch.createCountDownLatch(res.length, {timeout: 10 * 1000}, function() {
		      			if(!fails.length) {
							console.log('add blacklist successfully');
						} else {
							console.log('add blacklist failed with servers: %j', fails);
						}
						rl.prompt();
			 		});
			 		...
			 		for(var i=0; i<res.length; i++) {
						(function(index) {
							if(res[index].indexOf('connector') != -1){
