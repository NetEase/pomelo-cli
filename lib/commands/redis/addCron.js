...
Command.prototype.handle = function(agent, comd, argv, rl, client, msg) {
	...
	var cron_arg = {};
	var cmd = {command: module.exports.commandId, cron: cron_arg};
	var serverId;
	var serverType;
	var fails = [];
					...
					if(err) {
			      	...
			      	if(!res.length) {
			      		...
			      		var latch = countDownLatch.createCountDownLatch(res.length, {timeout: 10 * 1000}, function() {
			      			if(!fails.length) {
								console.log('addCron servers successfully for serverType: ' + serverType);
							} else {
								console.log('addCron failed with servers: %j', fails);
							}
							rl.prompt();
				 		});
